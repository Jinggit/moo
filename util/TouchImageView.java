...
public class TouchImageView extends ImageView {
    ...
    private class DoubleTapZoom implements Runnable {
        ...
        public void run() {
            ...
            if (t < ScaleAnimation.MAX_SCALE_FACTOR) {
        ...
        private float interpolate() {
            return this.interpolator.getInterpolation(Math.min(ScaleAnimation.MAX_SCALE_FACTOR, ((float) (System.currentTimeMillis() - this.startTime)) / ZOOM_TIME));
        }
    ...
    private class Fling implements Runnable {
        ...
        Fling(int velocityX, int velocityY) {
            ...
            TouchImageView.this.matrix.getValues(TouchImageView.this.f24m);
            int startX = (int) TouchImageView.this.f24m[2];
            int startY = (int) TouchImageView.this.f24m[5];
    ...
    private void sharedConstructing(Context context) {
        ...
        this.f24m = new float[9];
        this.normalizedScale = ScaleAnimation.MAX_SCALE_FACTOR;
        if (this.mScaleType == null) {
            this.mScaleType = ScaleType.FIT_CENTER;
        }
        this.minScale = ScaleAnimation.MAX_SCALE_FACTOR;
        this.maxScale = 3.0f;
    ...
    public boolean isZoomed() {
        return this.normalizedScale != ScaleAnimation.MAX_SCALE_FACTOR;
    }
    ...
    private void savePreviousImageValues() {
        if (this.matrix != null && this.viewHeight != 0 && this.viewWidth != 0) {
            this.matrix.getValues(this.f24m);
            this.prevMatrix.setValues(this.f24m);
    ...
    public Parcelable onSaveInstanceState() {
        ...
        this.matrix.getValues(this.f24m);
        bundle.putFloatArray("matrix", this.f24m);
    ...
    public void onRestoreInstanceState(Parcelable state) {
        if (state instanceof Bundle) {
            ...
            this.f24m = bundle.getFloatArray("matrix");
            this.prevMatrix.setValues(this.f24m);
    ...
    public void setZoom(float scale, float focusX, float focusY, ScaleType scaleType) {
        if (this.onDrawReady) {
            ...
            this.matrix.getValues(this.f24m);
            this.f24m[2] = -((getImageWidth() * focusX) - (((float) this.viewWidth) * 0.5f));
            this.f24m[5] = -((getImageHeight() * focusY) - (((float) this.viewHeight) * 0.5f));
            this.matrix.setValues(this.f24m);
            fixTrans();
            setImageMatrix(this.matrix);
            return;
        }
    ...
    private void fixTrans() {
        this.matrix.getValues(this.f24m);
        float transX = this.f24m[2];
        float transY = this.f24m[5];
    ...
    private void fixScaleTrans() {
        ...
        if (getImageWidth() < ((float) this.viewWidth)) {
            this.f24m[2] = (((float) this.viewWidth) - getImageWidth()) / 2.0f;
        }
        if (getImageHeight() < ((float) this.viewHeight)) {
            this.f24m[5] = (((float) this.viewHeight) - getImageHeight()) / 2.0f;
        }
        this.matrix.setValues(this.f24m);
    }
    ...
    private void fitImageToView() {
        ...
        if (drawable != null && drawable.getIntrinsicWidth() != 0 && drawable.getIntrinsicHeight() != 0 && this.matrix != null && this.prevMatrix != null) {
            ...
            switch (C06301.$SwitchMap$android$widget$ImageView$ScaleType[this.mScaleType.ordinal()]) {
                case 1:
                    scaleY = ScaleAnimation.MAX_SCALE_FACTOR;
                    scaleX = ScaleAnimation.MAX_SCALE_FACTOR;
                    break;
                case 2:
                    ...
                    break;
                case 3:
                    scaleY = Math.min(ScaleAnimation.MAX_SCALE_FACTOR, Math.min(scaleX, scaleY));
                    scaleX = scaleY;
                    break;
                case 4:
                    break;
            ...
            if (isZoomed() || this.imageRenderedAtLeastOnce) {
                ...
                this.prevMatrix.getValues(this.f24m);
                this.f24m[0] = (this.matchViewWidth / ((float) drawableWidth)) * this.normalizedScale;
                this.f24m[4] = (this.matchViewHeight / ((float) drawableHeight)) * this.normalizedScale;
                float transX = this.f24m[2];
                float transY = this.f24m[5];
    ...
    private void translateMatrixAfterRotate(int axis, float trans, float prevImageSize, float imageSize, int prevViewSize, int viewSize, int drawableSize) {
        if (imageSize < ((float) viewSize)) {
            this.f24m[axis] = (((float) viewSize) - (((float) drawableSize) * this.f24m[0])) * 0.5f;
        } else if (trans > 0.0f) {
            this.f24m[axis] = -((imageSize - ((float) viewSize)) * 0.5f);
        } else {
            this.f24m[axis] = -((((Math.abs(trans) + (((float) prevViewSize) * 0.5f)) / prevImageSize) * imageSize) - (((float) viewSize) * 0.5f));
        }
    }
    ...
    public boolean canScrollHorizontally(int direction) {
        ...
        if ((Math.abs(x) + ((float) this.viewWidth)) + ScaleAnimation.MAX_SCALE_FACTOR < getImageWidth() || direction <= 0) {
            return true;
        }
        return false;
    }
    ...
    private PointF transformCoordTouchToBitmap(float x, float y, boolean clipToBitmap) {
        ...
        float finalY = ((y - this.f24m[5]) * origH) / getImageHeight();
        if (clipToBitmap) {
    ...
    private PointF transformCoordBitmapToTouch(float bx, float by) {
        this.matrix.getValues(this.f24m);
        return new PointF(this.f24m[2] + (getImageWidth() * (bx / ((float) getDrawable().getIntrinsicWidth()))), this.f24m[5] + (getImageHeight() * (by / ((float) getDrawable().getIntrinsicHeight()))));
    }
